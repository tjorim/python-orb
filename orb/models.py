...
class BaseDatasetRecord(BaseModel):
    ...
    # Common dimensions
    network_type: Optional[int] = Field(
        None,
        description="Network interface type (0=unknown, 1=wifi, 2=ethernet, 3=other)",
    )
    network_state: Optional[int] = Field(
        None, description="Speed test load state during interval"
    )
    country_code: Optional[str] = Field(
        None, description="Geocoded 2-digit ISO country code"
    )
    ...
    location_source: Optional[int] = Field(
        None, description="Location Source (0=unknown, 1=geoip)"
    )
...
class Scores1mRecord(BaseDatasetRecord):
    ...
    score_version: Optional[str] = Field(
        None, description="Semantic version of scoring methodology"
    )

    # Measures
    orb_score: Optional[float] = Field(
        None, description="Orb Score over interval (0-100)"
    )
    responsiveness_score: Optional[float] = Field(
        None, description="Responsiveness Score over interval (0-100)"
    )
    reliability_score: Optional[float] = Field(
        None, description="Reliability Score over interval (0-100)"
    )
    speed_score: Optional[float] = Field(
        None, description="Speed (Bandwidth) Score over interval (0-100)"
    )
    speed_age_ms: Optional[int] = Field(
        None, description="Age of speed used in milliseconds"
    )
    lag_avg_us: Optional[float] = Field(
        None, description="Lag in microseconds (avg if interval)"
    )
    download_avg_kbps: Optional[int] = Field(
        None, description="Content download speed in Kbps"
    )
    upload_avg_kbps: Optional[int] = Field(
        None, description="Content upload speed in Kbps"
    )
    unresponsive_ms: Optional[float] = Field(
        None, description="Time spent in unresponsive state in Milliseconds"
    )
    measured_ms: Optional[float] = Field(
        None, description="Time spent actively measuring in Milliseconds"
    )
    lag_count: Optional[int] = Field(None, description="Count of Lag samples included")
    speed_count: Optional[int] = Field(
        None, description="Count of speed samples included"
    )
...
class ResponsivenessRecord(BaseDatasetRecord):
    ...
    network_name: Optional[str] = Field(
        None, description="Network name (SSID, if available)"
    )

    # Measures
    lag_avg_us: Optional[int] = Field(None, description="Avg Lag in microseconds")
    latency_avg_us: Optional[int] = Field(
        None, description="Avg round trip latency in microseconds"
    )
    jitter_avg_us: Optional[int] = Field(
        None,
        description="Avg Interpacket interarrival difference (jitter) in microseconds",
    )
    latency_count: Optional[float] = Field(
        None, description="Count of round trip latency measurements that succeeded"
    )
    latency_lost_count: Optional[int] = Field(
        None, description="Count of round trip latency measurements that were lost"
    )
    ...
    router_lag_avg_us: Optional[int] = Field(
        None, description="Avg router lag in microseconds"
    )
    router_latency_avg_us: Optional[int] = Field(
        None, description="Avg router round trip latency in microseconds"
    )
    router_jitter_avg_us: Optional[int] = Field(
        None, description="Avg router jitter in microseconds"
    )
    router_latency_count: Optional[float] = Field(
        None, description="Count of router latency measurements that succeeded"
    )
    router_latency_lost_count: Optional[int] = Field(
        None, description="Count of router latency measurements that were lost"
    )
    router_packet_loss_pct: Optional[float] = Field(
        None, description="Router packet loss percentage"
    )
...
class WebResponsivenessRecord(BaseDatasetRecord):
    ...
    # Measures
    ttfb_us: Optional[int] = Field(
        None, description="Time to First Byte loading a web page in microseconds"
    )
    dns_us: Optional[int] = Field(
        None, description="DNS resolver response time in microseconds"
    )
...
class SpeedRecord(BaseDatasetRecord):
    ...
    speed_test_engine: Optional[int] = Field(
        None, description="Testing engine (0=orb, 1=iperf)"
    )
    speed_test_server: Optional[str] = Field(
        None, description="Server URL or identifier"
    )
